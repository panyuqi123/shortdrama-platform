...
export default function DramaPage() {
  ...
  return (
    ...
    >
      {/* 返回 + 标题 */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-black via-black/80 to-transparent px-4 pt-4 pb-4">
        <div className="flex items-center gap-2.5">
          <button
            onClick={() => router.back()}
            className="w-10 h-10 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-md border border-white/10 hover:bg-black/70 transition-colors"
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white/40 text-xs">
              {formatViewCount(drama.viewCount)} 播放 · ★ {drama.rating}
            </p>
          </div>
          <button className="w-10 h-10 flex items-center justify-center rounded-full bg-black/50 backdrop-blur-md border border-white/10 hover:bg-black/70 transition-colors">
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="px-4 py-5">
        ...
        <div className="flex items-start gap-3 mb-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-neutral-500 text-xs">
              ...
              <div className="w-1 h-1 rounded-full bg-neutral-700" />
              <span>更新至 {drama.updatedAt}</span>
            </div>
          </div>
          <button
            onClick={() => setShowDesc(!showDesc)}
            className="flex items-center gap-1 text-red-500 text-xs font-medium bg-red-500/10 px-3 py-1.5 rounded-full border border-red-500/20 hover:bg-red-500/15 transition-colors"
          >
            {showDesc ? '收起' : '详情'}
        ...
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-white font-bold text-sm">
            选集
            <span className="text-neutral-600 font-normal text-xs ml-2">共 {drama.episodes.length} 集</span>
          </h3>
            ...
            return (
              ...
              >
                ...
                <div className={`absolute inset-0 flex items-center justify-center transition-all ${
                  !unlocked ? 'bg-black/60' : isCurrent ? 'bg-black/20' : 'bg-black/30'
                }`}>
                  {!unlocked ? (
                ...
                {episode.isFree && !isCurrent && (
                  <div className="absolute top-1 left-1 bg-emerald-500/90 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                    免费
                  </div>
                )}
                {/* 集数标签 */}
        ...
        <div className="grid grid-cols-3 gap-2">
          {mockDramas.filter(d => d.id !== drama.id).slice(0, 3).map(d => (
            <button
              key={d.id}
              onClick={() => window.location.href = `/drama/${d.id}`}
            ...
            >
              <div className="aspect-[3/4] relative">
                ...
                <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-sm text-white text-xs px-1.5 py-0.5 rounded border border-white/10">
                  {d.episodes.length}集
                </div>
                {d.isVipOnly && (
                  <div className="absolute top-2 left-2">
                    <span className="text-[10px] bg-yellow-400/90 text-black font-bold px-1.5 py-0.5 rounded">VIP</span>
                  </div>
                )}
              </div>
            </button>
      ...
      {/* 付费弹窗 - 优化版 */}
      {showPaywall && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowPaywall(false)}>
          ...
          <div className="relative w-full max-w-sm bg-neutral-900 rounded-t-2xl sm:rounded-2xl overflow-hidden border border-white/5 shadow-2xl">
            ...
            <div className="p-6 pb-8">
              ...
              <div className="flex items-center gap-4 mb-5">
                <div className="w-16 h-22 rounded-xl overflow-hidden shadow-lg ring-1 ring-white/10">
                ...
                <div>
                  ...
                  <h3 className="text-white font-bold text-base leading-tight mb-1">
                    {drama.title} · 第{currentUnlock !== null ? drama.episodes[currentUnlock].number : ''}集
                  </h3>
                ...
                >
                  <div>
                    <p className="text-white font-bold text-sm flex items-center gap-1.5">
                      🪙 金币解锁
                      <span className="text-neutral-500 font-normal text-xs">第{currentUnlock !== null ? drama.episodes[currentUnlock].number : ''}集</span>
                    </p>
              ...
              <button onClick={() => setShowPaywall(false)} className="w-full text-neutral-600 text-sm py-2 mt-3 hover:text-neutral-400 transition-colors">
                稍后再说
              </button>
            </div>
          </div>
