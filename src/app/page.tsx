...
export default function HomePage() {
  ...
  const handleSendComment = () => {
    ...
    const newComment = {
      id: Date.now(), user: '我', avatar: '?seed=me',
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diffY) > Math.abs(diffX) && Math.abs(diffY) > 50) {
      if (diffY > 0 && currentIdx < filteredDramas.length - 1) setCurrentIdx(p => p + 1)
      else if (diffY < 0 && currentIdx > 0) setCurrentIdx(p => p - 1)
    }
  }
  ...
  return (
    <div
      className="h-dvh w-screen overflow-hidden bg-black select-none relative no-select"
    ...
    >
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute bottom-36 left-4 right-20">
            ...
            <div className="flex items-center gap-4 mt-2.5 text-white/40 text-xs">
              <span className="flex items-center gap-1">
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/><path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10z" clipRule="evenodd"/></svg>
          ...
          <div className="absolute bottom-32 right-3 flex flex-col items-center gap-5">
            <div className="flex flex-col items-center">
              <div className="w-13 h-13 rounded-full overflow-hidden border-[2.5px] border-white shadow-lg">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${currentDrama.id}`} className="w-full h-full object-cover" />
              </div>
              <div className="w-5 h-5 bg-red-500 rounded-full -mt-2.5 flex items-center justify-center text-white text-xs font-bold shadow-md border-2 border-white">+</div>
            </div>

            <div className="flex flex-col items-center">
              <button onClick={handleLike} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-90 shadow-lg ${liked ? 'bg-red-500 shadow-red-500/30' : 'bg-white/10 backdrop-blur-md border border-white/10'}`}>
                <svg className={`w-6 h-6 transition-all ${liked ? 'text-white scale-110' : 'text-white'}`} fill={liked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex flex-col items-center">
              <button onClick={() => setShowComments(true)} className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center active:scale-90 shadow-lg">
          ...
          <div className="absolute top-1/2 left-2 -translate-y-1/2 flex flex-col gap-2 z-10">
            {filteredDramas.map((_, i) => (
              <div key={i} className={`rounded-full transition-all duration-300 ${i === currentIdx ? 'w-1.5 h-6 bg-white shadow-sm' : 'w-1 h-1.5 bg-white/30'}`} />
            ))}
          </div>
        </div>
      )}
      ...
      {showComments && (
        <div className="fixed inset-0 z-50 flex flex-col bg-neutral-900 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-neutral-800/50 flex-shrink-0"
               style={{ paddingTop: 'max(env(safe-area-inset-top), 12px)' }}>
            <button onClick={() => setShowComments(false)} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 overscroll-contain">
            {comments.map(comment => (
              <div key={comment.id} className="flex gap-3">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg/${comment.avatar}`} className="w-10 h-10 rounded-full flex-shrink-0" />
          ...
          <div className="px-4 py-3 border-t border-neutral-800/50 bg-neutral-900/95 flex-shrink-0"
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleSendComment} disabled={!commentText.trim()}
                      className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-sm font-semibold px-5 py-3 rounded-full disabled:opacity-30 hover:brightness-110 active:scale-95 transition-all flex-shrink-0">
                发送
              </button>
            </div>
          </div>
      ...
      {/* 顶部区域 */}
      <div
        className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/90 via-black/60 to-transparent"
      ...
      >
        <div className="flex items-center justify-between mb-3.5 px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-black text-lg">Reel<span className="text-red-500">Short</span></span>
          </div>
          <div className="flex items-center gap-3">
            <button className="w-9 h-9 rounded-full bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center active:scale-90 transition-transform">
            ...
            <button onClick={() => setActiveTab('me')} className="w-9 h-9 rounded-full overflow-hidden border-2 border-white/20 shadow-md active:scale-95 transition-transform">
              {user ? (
                <img src={user.avatar} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-neutral-800 flex items-center justify-center">
                  <svg className="w-5 h-5 text-white/40" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
                </div>
              )}
            </button>
          </div>
        ...
        {/* 分类标签 */}
        <div className="flex gap-2 overflow-x-auto pb-1 px-4 -mx-4 scroll-smooth">
          {categories.map(cat => (
...
function MeTab({ onBack }: { onBack: () => void }) {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh px-8 text-center overflow-hidden relative">
        ...
        <div className="w-22 h-22 rounded-full bg-neutral-800/80 flex items-center justify-center text-5xl mb-6 shadow-xl ring-1 ring-white/10">
          <svg className="w-11 h-11 text-neutral-600" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
        </div>
        ...
        <button onClick={onBack} className="text-neutral-600 text-sm mt-5 hover:text-white transition-colors">返回首页</button>
      </div>
    )
  }

  return (
    <div className="min-h-dvh bg-neutral-950 overflow-x-hidden overflow-y-auto"
         style={{ paddingTop: 'max(env(safe-area-inset-top), 12px)' }}>
      <div className="absolute top-0 left-0 right-0 h-40 bg-gradient-to-b from-red-500/8 to-transparent pointer-events-none" />

      <button onClick={onBack}
        className="flex items-center gap-2 text-white/50 text-sm px-4 mb-4 hover:text-white transition-colors z-10 relative">
      ...
      <div className="bg-neutral-900/80 backdrop-blur-md rounded-2xl p-5 mx-4 mb-4 border border-white/5 shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <button onClick={logout} className="text-neutral-500 text-xs px-3 py-1.5 rounded-full bg-neutral-800/80 border border-neutral-700/50 hover:bg-neutral-700 transition-colors">退出</button>
        </div>

        {!user.isVip && (
          <button onClick={buyVip} className="mt-4 w-full bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-xl py-3.5 text-sm font-bold shadow-lg shadow-red-500/20 hover:brightness-110 active:scale-[0.99] transition-all">
            开通 VIP · 全站免费看
          </button>
        )}
      ...
      <div className="bg-neutral-900/80 backdrop-blur-md rounded-2xl mx-4 overflow-hidden border border-white/5 shadow-xl">
        {[
          ...
          <button key={item.label}
            ...
            <svg className="w-4 h-4 text-neutral-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
          </button>
        ))}
      </div>
